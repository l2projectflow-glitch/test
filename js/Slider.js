class Slider {
	constructor(params) {
		...
		this._interval = null;  // Novo: variável para armazenar o intervalo
		this._init();
	}
	...
	_changeSlide(direction) {
		...
		// Corrige os limites para permitir o loop automático
		if (this._currentIndex >= this._data.length) {
			this._currentIndex = 0; // Volta ao primeiro slide
		}

		if (this._currentIndex < 0) {
			this._currentIndex = this._data.length - 1; // Vai ao último slide
		}
	...
	// Novo método para iniciar o slider automático
	_startAutoSlide() {
		this._interval = setInterval(() => {
			this._changeSlide(-1); // Muda para o próximo slide
		}, 3000); // 3 segundos de intervalo
	}

	// Novo método para parar o slider automático (caso necessário)
	_stopAutoSlide() {
