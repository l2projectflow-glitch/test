...
class News {
	...
	public static function listNews($pgBeg, $pgMax, $buscar='') {
		...
		$sql = "SELECT * FROM (SELECT ROW_NUMBER() OVER (ORDER BY post_date DESC) AS row, t1.* FROM site_news AS t1) AS t2 WHERE t2.row BETWEEN ".($pgBeg+1)." AND ".($pgBeg+$pgMax)."";
		return DB::Executa($sql, "SITE");
	...
	public static function editNew($nid, $post_date, $vis, $title_pt, $content_pt, $title_en, $content_en, $title_es, $content_es, $imagem) {
		...
		$sql = "UPDATE site_news SET ".((!empty($imagem)) ? "img = '".$imagem."'," : "")." post_date = '".$post_date."', vis = '".$vis."', title_pt = '".$title_pt."', title_en = '".$title_en."', title_es = '".$title_es."', content_pt = '".$content_pt."', content_en = '".$content_en."', content_es = '".$content_es."' WHERE nid = '".$nid."'";
		return DB::Executa($sql, "SITE");
	...
	public static function findNew($nid) {
		...
		$sql = "SELECT TOP 1 * FROM site_news WHERE nid = '".$nid."'";
		return DB::Executa($sql, "SITE");
	...
	public static function deleteNew($nid) {
		...
		$sql = "DELETE FROM site_news WHERE nid = '".$nid."'";
		return DB::Executa($sql, "SITE");
