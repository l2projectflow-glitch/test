...
$editar = Banners::editBanner($bid, $imgurl_pt, $imgurl_en, $imgurl_es, $link, $target, $vis);
if($editar){
	
	if(file_exists('../'.$dir_banners.$findBanner[0]['imgurl_pt']) && !empty($findBanner[0]['imgurl_pt']) && !empty($imgurl_pt)) {
		@unlink('../'.$dir_banners.$findBanner[0]['imgurl_pt']);
	}
	
	if(file_exists('../'.$dir_banners.$findBanner[0]['imgurl_en']) && !empty($findBanner[0]['imgurl_en']) && !empty($imgurl_en)) {
		@unlink('../'.$dir_banners.$findBanner[0]['imgurl_en']);
	}
	
	if(file_exists('../'.$dir_banners.$findBanner[0]['imgurl_es']) && !empty($findBanner[0]['imgurl_es']) && !empty($imgurl_es)) {
		@unlink('../'.$dir_banners.$findBanner[0]['imgurl_es']);
	}
	
	adminLog("Editou um banner (ID ".$bid.")"); // Admin Log
	fim('Banner editado com sucesso!', 'OK', './?page=list&module=banners');
