...
$coinQntV = !empty($coinQntV) ? trim($coinQntV) : 1;

if($coinQntV <= 0) {
	fim($LANG[12055].' #QNT', 'ERROR', './?module=donate&page=add');
}

if($coinPrice <= 0) {
	fim($LANG[12055].' #PRC', 'ERROR', './?module=donate&page=add');
}

$valor = (intval(trim($qtdCoins)) * (trim($coinPrice)/trim($coinQntV)));

if($valor == 0) {
	fim($LANG[12055].' #VAL', 'ERROR', './?module=donate&page=add');
}
...
if($insertDonation) {
	...
	fim($LANG[12055].' #DB', 'ERROR', './?module=donate&page=add');
}
