...
if($funct['gamst7'] != 1) { fim($LANG[40003], 'ERROR', './'); }
?>
...
<div style='display:table; width: 710px; margin: 0 auto;'>
	...
	if($genNew) {
		...
		if(count($query) > 0) {
			...
			for($i=0, $c=count($query); $i < $c; $i++) {
				...
				$war_day = ($query[$i]['sdate'] == '0' ? '00/00/0000 00:00' : (strlen($query[$i]['sdate']) > 11 ? date('d F, Y H:i', ($query[$i]['sdate']/1000)-$gmtf) : date('d F, Y H:i', ($query[$i]['sdate'])-$gmtf)));
				...
				$line .= "\n<line>\n";
				$line .= "<castle>".strtr(strtolower($query[$i]['name']), array('_castle' => '', 'innadrile' => 'innadril', 'godad' => 'goddard', 'schutt' => 'schuttgart'))."</castle>\n";
